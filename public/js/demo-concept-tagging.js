...
function getAbstractConcepts() {
  ...
  if(text != previousText){
    $.post('/api/alchemyConceptTagging', {
        text: text
      })
      .done(function(results) {
        ...
        for (var i = 0; i < results.concepts.length; i++) {
          if (check_duplicate_concept(unique_concept_array, results.concepts[i].text) || unique_concept_array.length == 3)
            continue;
          else
            unique_concept_array.push(results.concepts[i].text);

          var abstract_concept_div = '<div class="concept--abstract-concept-list-container"><span class="concept--abstract-concept-list-item" concept_id="' + results.concepts[i].text + '">' + results.concepts[i].text + '</span></div>';
