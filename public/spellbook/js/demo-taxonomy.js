  ...
  if(text != previousText){
    $.post('/api/alchemyTaxonomy', {
      ...
      .done(function(results) {
        ...
        if (results.taxonomy.length)
          $('.concept--abstract-concept-title').addClass('active');

        for (var i = 0; i < results.taxonomy.length; i++) {
          if (check_duplicate_concept(unique_concept_array, results.taxonomy[i].label) || unique_concept_array.length == 3)
            continue;
          else
            unique_concept_array.push(results.taxonomy[i].label);

          var abstract_concept_div = '<div class="concept--abstract-concept-list-container"><span class="concept--abstract-concept-list-item" concept_id="' + results.taxonomy[i].label + '">' + results.taxonomy[i].label + '</span></div>';
